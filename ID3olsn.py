...
flashcards_def = [
    ...
    {"id": 3, "q": "What is 5 x 5?", "a": "25", "state": "learning", "step": 0, "days_till_review": 0, "review_count": 0}
]
...
with conn:
    c.execute("""
        CREATE TABLE IF NOT EXISTS vars(
            ...
            study_amount INTEGER DEFAULT 20
    )
    """)
...
class Card_Manager():
    ...
    def load_db(self):
        ...
        # UPDATE: Fetch both current_day and study_amount
        c.execute("SELECT current_day, study_amount FROM vars WHERE id = 1")
        result = c.fetchone()
        if result:
            self.current_day = result[0]
            self.study_amount = result[1]
        else: 
            self.current_day = 0
    ...
    def save_db(self, card_id, new_state, new_step, new_days_till_review, new_review_count):
        with conn:
            ...
            c.execute("UPDATE vars SET study_amount = ? WHERE id = 1", (self.study_amount,))
    ...
    # Add these inside your Card_Manager class
    def log_interaction(self):
        with conn:
            # Ensure the day exists in the table, then add 1 to the count
            c.execute("INSERT OR IGNORE INTO activity (day, count) VALUES (?, 0)", (self.current_day,))
            c.execute("UPDATE activity SET count = count + 1 WHERE day = ?", (self.current_day,))

    def get_activity(self):
        c.execute("SELECT day, count FROM activity")
        return dict(c.fetchall()) # Returns a dictionary like {0: 5, 1: 12, 2: 0}
...
class Session():
    def __init__(self):
        ...
        # UPDATE: Pull the amount from the database via Card_Manager
    ...
    def build_up(self):
        ...
        for c in Card.new_cards[:int(Card.study_amount/4)]:
            self.wani_queue.append(c)
        for c in Card.old_cards[:int(Card.study_amount)]:
            self.wani_queue.append(c)
            
        # Anki Queue: Gets review cards
        if Card.review_cards:
            target = int(Card.study_amount*4)
            added = 0
    ...
    def advance_day(self):
        ...
        if len(session.anki_queue) > 0:
            for i in session.anki_queue:
                card_list = list(i)
                card_list[5] = 1
                card_list[6] = card_list[6] - 1
                Card.save_db(card_list[0], card_list[3], card_list[4], card_list[5], card_list[6])
    ...
    with home_container:
        ...
        def home_ui():
            ...
            with ui.row().classes("w-full gap-4 justify-between mt-4"):
                ...
                # --- NEW: SETTINGS CARD ---
            with ui.column().classes("flex-1 sketch-card p-4 items-center gap-2"):
                ui.label("Settings").classes("font-bold text-slate-800 text-lg")
                ui.icon('settings', size='3.5rem').classes('text-slate-300 my-2')
                    
                def on_amount_change(e):
                    val = int(e.value or 0)
                    Card.study_amount = val
                    Card.save_db()
                        
                ui.number("Study Limit", value=Card.study_amount, on_change=on_amount_change).classes("w-full bg-slate-50 px-2")
            # --- NEW: HEATMAP CARD ---
            with ui.column().classes("w-full sketch-card p-4 mt-4"):
                ui.label("Activity Heatmap").classes("font-bold text-slate-800 text-lg mb-2")
                
                activity_data = Card.get_activity()
                
                with ui.row().classes('w-full gap-1 flex-wrap'):
                    # Display the last 30 days (or from Day 0 if you haven't reached 30 days yet)
                    start_day = max(0, Card.current_day - 29)
                    
                    for d in range(start_day, Card.current_day + 1):
                        count = activity_data.get(d, 0)
                        
                        # Determine the color based on how many cards were answered
                        if count == 0: bg_color = 'bg-slate-200'
                        elif count < 5: bg_color = 'bg-emerald-200'
                        elif count < 15: bg_color = 'bg-emerald-400'
                        else: bg_color = 'bg-emerald-600'
                        
                        # Draw the square with a tooltip showing the exact count
                        ui.label().classes(f'w-4 h-4 rounded-sm {bg_color}').tooltip(f"Day {d}: {count} cards")
...
ui.run(title="Ankers Hub", host="172.31.46.7", port=8080, show=False)